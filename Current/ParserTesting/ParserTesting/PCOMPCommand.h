...
struct PCOMPCommand : GenericCommand, virtual IPCOMPCommand {
	...
	void PostProcess()
	{
		...
		{
			auto layerCount = m_data.size();
			for (auto layer = layerCount - 1; layer >= 0; layer--)
			{
	...
	VisitableFieldData* VisitField(int index, VisitableFieldData& fieldData) override
	{
		...
		default:
		{
			...
			case 4:
			{
				PCOMPData data;
				m_data.push_back(data);
				m_currentDataIndex = static_cast<int>(m_data.size()) - 1;
	...
	unsigned int Count() const override { return static_cast<unsigned int>(m_data.size()); }
